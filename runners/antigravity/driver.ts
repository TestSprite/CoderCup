/**
 * Antigravity driver - run(input: TaskRunInput): Promise<RunManifest>.
 *
 * Mirrors the Claude / Codex drivers' structure. Antigravity-specific
 * delta is in invoke.ts (warmup probe + `agy --print` invocation) and
 * parse-output.ts (Antigravity's structured-or-prose output).
 *
 * Antigravity CLI was released 6 days before this driver (Google I/O
 * 2026-05-19); the warmup probe in invoke.ts is the early-fail guard
 * against the CLI being broken or unauthenticated on the runner.
 */
...
import { invokeAgy } from './invoke';
import { createAgyParser } from './parse-output';
...
const AGY_DEFAULT_MODEL = 'gemini-3-pro';
...
export async function run(input: TaskRunInput): Promise<RunManifest> {
  ...
  const meter = new UsageMeter(AGY_DEFAULT_MODEL);
  ...
  const parseLine = createAgyParser(AGY_DEFAULT_MODEL, {
    onEvent: (event) => {
  ...
  const manifest: RunManifest = {
    ...
    driver_metadata: {
      driver: 'antigravity_cli' as const,
      model_id: AGY_DEFAULT_MODEL,
      ...
      notes: [
        ...
        deploy.stubbed
          ...
          : deploy.jobStatus !== 'SUCCEED'
            ...
            : null,
        'agy CLI v0.x; output format may evolve. Tokens may be estimated from compute_units (5k tokens/unit heuristic).',
        result.exitCode === 127 ? 'WARMUP_PROBE_FAILED' : null,
